...
export default function TransactionsPage() {
  ...
  }, [type, medicineId, from, to, facilityId, isAdmin, page]);
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead className="border-b bg-slate-50 text-sm text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium">Source Facility</th>
              <th className="px-3 py-2 text-left font-medium">Destination Facility</th>
            ...
            {loading ? (
              ...
              data.transactions.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2 text-slate-600 text-sm">{t.sourceFacility?.name ?? "—"}</td>
